...
One limit remains. No statement or transaction lock timeout backs the discipline up: Postgres's deadlock detector (1s by default) catches genuine cycles, but a future one-directional pile-up would block rather than fail fast. A `lock_timeout` was considered as a backstop and left out for now rather than risk turning load-test contention into spurious failures — it is the obvious next hardening if the ordering ever needs enforcing rather than documenting.

## The seat-versus-seat residual, closed in M8 (#16)

This ADR originally disclosed a second limit: the sweep's bulk `UPDATE` locked many `event_seats` rows in whatever order the plan scanned them, which is not the ascending-id order every per-seat loop uses. That made seat-versus-seat ordering between the sweep and a multi-seat hold deadlock-prone independently of the table ordering fixed here. It is now closed, and the rule stated at the top of this ADR — **ascending id order**, not merely "seats before holds" — applies to the sweep as literally as it does to the loops.

The fix is worth recording because the obvious version of it does not work. Selecting the target ids with an `ORDER BY` and then issuing `UPDATE ... WHERE id IN (:ids)` changes nothing: an unlocked `SELECT` acquires no row locks, so ordering it orders nothing, and the planner remains free to reorder the `UPDATE`'s `IN` list. That shape produces two statements, an ordering comment, and the identical bug — which is worse than the bug alone, because it reads as fixed.

The locks have to be taken by the ordering statement itself. `SELECT id ... ORDER BY es.id FOR UPDATE OF es` puts Postgres's row-locking step above the sort in the plan, so rows are locked in ascending id order; the bulk `UPDATE` that follows only re-touches rows this same transaction already holds, so its own scan order stops mattering. Two details are load-bearing: it must be `FOR UPDATE OF es` rather than bare `FOR UPDATE`, because the predicate joins `holds` and a bare `FOR UPDATE` would take the `holds` row locks *in the same statement* — violating this ADR's main rule while fixing its residual — and it is a scalar id projection, never a materialized entity, per ADR-0010.
