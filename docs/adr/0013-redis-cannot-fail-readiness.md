...
That leaves a custom `HealthIndicator` bean named `redis` that always reports `UP` and carries real reachability in its details — rejected, because it would make the parent aggregate state something false. An operator reading `/actuator/health` would see `"redis":{"status":"UP"}` while Redis was down, and would have to know to look inside the details to find out otherwise. A health endpoint is the one surface in a system whose entire purpose is to state what is true; making it report `UP` for a component that is `DOWN` trades a small configuration convenience for a false statement, and would also mean the architecture lived in a Java class instead of in one readable line.

(An earlier draft of this paragraph framed that as something an *anonymous* caller would see. That was wrong, and the correction matters for understanding the actual exposure: under ADR-0012 the parent aggregate is authenticated, and Redis is not a member of either public group, so no anonymous caller ever sees a `redis` component at all — truthful or otherwise. The rejection stands regardless of audience; a health endpoint should not lie to anyone, including an operator holding a token.)
...
(**Amended by ADR-0014.** The claim that Postgres-down cannot be tested the way Redis-down is was true only while the health check shared the application datasource. Once the readiness probe took its own connection path, a test could point *that* path at a dead port while the main datasource stayed real, so the outage became observable in-process after all. The paragraph above still describes why it was untestable at the time, and the conclusion it draws — that a test must state which half it proves — holds more strongly than before, since the in-process test covers the refused case only. See ADR-0014's consequences.)
